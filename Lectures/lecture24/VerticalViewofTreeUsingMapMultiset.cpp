...
void dfs(TreeNode* root, int col , int level, map<pair<int, int>, multiset<int>>&mp){
    if(root==nullptr){
        return;
    }
...
vector<int>VerticalViewOfTreeUsingMapAndMultiset(TreeNode* root){
    vector<int> ans;
    if(root==nullptr){
        return ans;
    }
    ...
    for(pair<pair<int, int>, multiset<int>> x:mp){
...
int main(){
    ...
    bfs(root);
    cout << "-----------------------------" << endl;
    vector<int> ans = VerticalViewOfTreeUsingMapAndMultiset(root);
    for(int x:ans){
        cout << x << " "; 
    }
    cout << endl << "-------------------------" << endl;
